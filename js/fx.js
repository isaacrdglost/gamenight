...
window.FX = (function(){
  ...
  /* -------- os 5 efeitos -------- */
  function airhorn(a,saida){
    const t=a.currentTime, g=a.createGain(); g.connect(saida);
  ...
  function rimshot(a,saida){
    ...
    function tom(when,freq){
      ...
      o.connect(g); g.connect(saida); o.start(when); o.stop(when+.15);
    }
    ...
    s.connect(hp); hp.connect(g); g.connect(saida); s.start(t+.3); s.stop(t+.62);
  }
  function trombone(a,saida){
    ...
    notes.forEach((f,i)=>{
      ...
      o.connect(lp); lp.connect(g); g.connect(saida); o.start(when); o.stop(when+dur); when+=dur*.85;
    });
  }
  function ding(a,saida){
    const t=a.currentTime;
    [880,1320].forEach((f,i)=>{
      ...
      o.connect(g); g.connect(saida); o.start(st); o.stop(st+.5);
    });
  }
  function erro(a,saida){
    const t=a.currentTime;
    [0,.18].forEach(off=>{
      ...
      o.connect(g); g.connect(saida); o.start(st); o.stop(st+.17);
    });
  }
  /* -------- sons "normais" de partida (discretos, pra dar dinâmica) -------- */
  function pop(a,saida){ // nova rodada / pergunta aparece
    ...
    o.connect(g); g.connect(saida); o.start(t); o.stop(t+.18);
  }
  function chime(a,saida){ // revelação
    const t=a.currentTime;
    [659,988].forEach((f,i)=>{ const o=a.createOscillator(),g=a.createGain(); o.type="sine"; o.frequency.value=f; const st=t+i*.1;
      ...
      o.connect(g); g.connect(saida); o.start(st); o.stop(st+.42); });
  }
  function fanfarra(a,saida){ // vitória
    ...
    notes.forEach((f,i)=>{ const st=t+i*.12, o=a.createOscillator(), g=a.createGain(); o.type="triangle"; o.frequency.value=f; const ultimo=i===notes.length-1;
      ...
      o.connect(g); g.connect(saida); o.start(st); o.stop(st+.7); });
  }
  function msg(a,saida){ // chat: bem curtinho, só pra sinalizar
    const t=a.currentTime;
    [740,1108].forEach((f,i)=>{
      ...
      o.connect(g); g.connect(saida); o.start(st); o.stop(st+.14);
    });
  }
  const SONS = { airhorn, rimshot, trombone, ding, erro, pop, chime, fanfarra, msg };
  // toca(nome, vol) — vol 1 = normal, 0.5 = metade (usado no eco da sala)
  function toca(nome, vol){
    if(mudo) return;
    const a=ctx(); if(!a) return;
    try{
      const mestre=a.createGain();
      mestre.gain.value = (vol==null?1:Math.max(0,Math.min(1,vol)));
      mestre.connect(a.destination);
      (SONS[nome]||(()=>{}))(a, mestre);
    }catch(e){}
  }
  ...
  function montar(){
    ...
    lista.forEach(([nome,emo,rot])=>{
      ...
      b.addEventListener("click",()=>{
        toca(nome);                                   // quem apertou ouve normal
        if(window.Chat && window.Chat.emitirSfx) window.Chat.emitirSfx(nome); // e ecoa na sala
      });
      grupo.appendChild(b);
    });
